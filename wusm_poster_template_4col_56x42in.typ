...
//
//  Landscape, 56 in x 42 in, 0.75 in margins, four-column layout.
//
...
#let box-spacing  = 0.32in
#let body-size    = 34pt
#let heading-size = 52pt
...
//  Title band
// ---------------------------------------------------------------------------
#let title-gutter = -2.6in
#let title-col-w  = 43in // controls the width of the center block
#let side-col-w   = (poster-w - 2 * poster-margin - title-col-w - 2 * title-gutter) / 2

#let left-logo-h =4in
#let lab-logo-h  = 2.2in
#let qr-gap      = 0.6in
#let qr-size     = (side-col-w - qr-gap) / 3
...
#let title-band(title: [], authors: [], affiliations: []) = block(
  ...
  #grid(
    ...
    // center: title / authors / affiliations
    {
      ...
      set par(spacing: 0em)
      {
        set par(justify: false, leading: 0.28em)
        set text(font: title-font, size: 140pt, weight: "bold", fill: black)
        title
      }
      v(2em)
      {
        set par(justify: false)
        set text(font: title-font, size: 64pt, weight: "semibold", fill: ink)
        authors
      }
      v(0.6em)
      {
        set par(justify: false)
        set text(font: heading-font, size: 40pt, fill: ink)
        affiliations
      }
    },
...
#block(height: poster-content-h, width: 100%, breakable: false)[
  #grid(
    ...
    grid(
      ...
      column-body(
        section([Result 2])[
          #fig-placeholder(10in, [Figure 2])
          #lorem(120)
        ],
        section-fill([Result 3])[
          #fig-placeholder(8in, [Figure 3])
          #lorem-fill()
        ],
      ),
      ...
      column-body(
        section([Result 4])[
          #lorem(120)
          #fig-placeholder(6.9in, [Figure 4])
          #lorem(80)
        ],
        section-fill([Result 5])[
          #fig-placeholder(5.5in, [Figure 5])
          #lorem-fill()
        ]
      ),
